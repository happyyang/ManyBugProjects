...
public abstract class BasePagingFrameAdapter<T> extends BaseFrameAdapter<T> implements AdapterPageChangedListener {
	...
	protected final int VIEW_TYPE_CONTENT = 0;
	protected final int VIEW_TYPE_LOADING = 1;
	protected final int VIEW_TYPE_LOADING_COMPLATE = 2;
	protected final int VIEW_TYPE_LOADING_TAP_NEXT = 3;
	...
	private int mLoadingStuats = STATUS_LOADING;
	...
	public int getItemViewType(int position) {
		if (position < getCount() - 1 || pagingListener == null) {
			...
			switch (mLoadingStuats) {
			case STATUS_LOADING:
				return VIEW_TYPE_LOADING;
			case STATUS_LOADING_NEED_TAP:
				return VIEW_TYPE_LOADING_TAP_NEXT;
			case STATUS_LOADING_END:
				return VIEW_TYPE_LOADING_COMPLATE;
			default:
				return VIEW_TYPE_LOADING;
			}
		}
	 ...
	 * 
	 * @param viewType
	 *            VIEW_TYPE_LOADING, VIEW_TYPE_LOADING_COMPLATE,
	...
	public void tapNextPage() {
		isLoadingNextPage = false;
		mLoadingStuats = STATUS_LOADING_NEED_TAP;
		notifyDataSetChanged();
	}
	...
	public void noMorePage() {
		isLoadingNextPage = false;
		mLoadingStuats = STATUS_LOADING_END;
		notifyDataSetChanged();
	}
	 ...
	 * 设置分页监听器
	 * 
	 * @param pagingListeners
	 */
	public void setOnPagingListener(PagingListener<T> pagingListeners) {
		this.pagingListener = pagingListeners;
		// page++;
		if (getCount() == 0) {
	...
	@Override
	protected void onViewAttach(int position, T item, View convertView) {
		// Empty module
	}
}
